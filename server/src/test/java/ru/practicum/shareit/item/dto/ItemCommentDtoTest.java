...
public class ItemCommentDtoTest {
    ...
    void testItemCommentDto() throws Exception {
        ItemCommentDto itemCommentDto = new ItemCommentDto(1L, "Молоток", "Молотит", true,
                new ItemRequestInItemDto(1L, "Молоток до 2 кг", localDateTime,
                        new UserDto(1L, "Егор", "egor.yandex.ru")),
                localDateTime, localDateTime,
                List.of(new CommentDto(1L, "Хороший молоток", "Егор", localDateTime)));

        JsonContent<ItemCommentDto> result = json.write(itemCommentDto);
        ...
        assertThat(result).extractingJsonPathStringValue("$.nextBooking").isEqualTo(localDateTime.toString());
        assertThat(result).extractingJsonPathStringValue("$.lastBooking").isEqualTo(localDateTime.toString());
        assertThat(result).extractingJsonPathNumberValue("$.comments[0].id").isEqualTo(1);
        assertThat(result).extractingJsonPathStringValue("$.comments[0].text").isEqualTo("Хороший молоток");
        assertThat(result).extractingJsonPathStringValue("$.comments[0].authorName").isEqualTo("Егор");
        assertThat(result).extractingJsonPathStringValue("$.comments[0].created").isEqualTo(localDateTime.toString());
    }
}
